...
class Model:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope("FC"):
            ...
            flat = tf.reshape(dropout3, [-1, 9 * 63 * 128])
...
class Model_2:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            self.X = tf.placeholder(tf.float32, [None, len(ALPHABET), CHAR_MAX_LENGTH, 1])
            ...
            # Convolutional Layer #3 and Pooling Layer #3
            conv4 = tf.layers.conv2d(inputs=dropout2, filters=512, kernel_size=[3, 3],
                                     padding="same", activation=tf.nn.relu)
            ...
            # Dense Layer with Relu
            flat = tf.reshape(dropout4, [-1, 512 * 9 * 63])
